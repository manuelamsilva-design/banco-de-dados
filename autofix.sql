...
CREATE TABLE ordens_servico (
    ...
    CONSTRAINT fk_os_mecanico  FOREIGN KEY (mecanico_id) REFERENCES mecanicos(id) ON DELETE RESTRICT
);
CREATE TABLE pecas_os (
    ...
    CONSTRAINT fk_peca_os  FOREIGN KEY (os_id) REFERENCES ordens_servico(id) ON DELETE CASCADE
);
INSERT INTO clientes (nome, email, telefone, cpf) VALUES 
('Manuela', 'manuela@email.com', '(48) 99911-2233', '11122233344'),
('Gabriel', 'gabriel@email.com', '(48) 98822-4455', '55566677788'),
('Laura', 'laura@email.com', '(48) 97733-6677', '99900011122');

INSERT INTO mecanicos (nome, especialidade, valor_hora) VALUES 
('Emanuel', 'Motor e Câmbio', 120.00),
('Marcio', 'Suspensão e Freios', 85.00),
('Mateus', 'Elétrica e Injeção', 100.00);
...
JOIN veiculos v ON os.veiculo_id = v.id
JOIN mecanicos m ON os.mecanico_id = m.id
JOIN pecas_os p ON os.id = p.os_id
...
FROM mecanicos m
JOIN ordens_servico os ON m.id = os.mecanico_id AND os.status = 'Concluida'
